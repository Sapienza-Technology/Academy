...
from dueruotesterz3 import Motore
from dueruotesterz3 import Ruote
import rospy
...
def callback(data, args):
    ...
    args[0].velocity(data.linear.y)
    args[1].avanti_dietro(data.linear.y)
    args[1].velocity(data.linear.y)
    args[2].avanti_dietro(-data.linear.y)
    args[2].velocity(-data.linear.y)
    args[3].avanti_dietro(data.linear.y)
    args[3].velocity(data.linear.y)
    
    args[4].inclinazione(data.linear.x)
    args[5].inclinazione(data.linear.x)
    
    args[6].inclinazione(data.linear.x)
    args[7].inclinazione(data.linear.x)
...
def listener():
    ...
    ruoteFront_Right=Ruote(6)
    ruoteFront_Left=Ruote(12)
    ruoteBack_Left=Ruote(23)
    ruoteBack_Right=Ruote(18)
    
   
    #rospy.Subscriber("/cmd_vel", Twist, callback, (motoreFront_Right,motoreFront_Left,motoreBack_Right,motoreBack_Left))
    rospy.Subscriber("motors_pub", Twist, callback, (motoreFront_Right,motoreFront_Left,motoreBack_Right,motoreBack_Left,ruoteFront_Right,ruoteFront_Left,ruoteBack_Left
,ruoteBack_Right))
